												...
												<tbody>
													<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
														...
														<td>
															...
															<a href="#"><?php echo ($vo["admin_name"]); ?></a>
														</td>
														<td><?php echo ($vo["admin_passwd"]); ?></td>
														<td><?php echo ($vo["type"]); ?></td>

														<td class="hidden-480">
															<span class="label label-sm label-success"><a href='#' target=_blank/>  <?php echo ($vo["tips"]); ?>  </a></span>
														</td>
							...
							<div class="col-xs-12">
								...
								<form class="form-horizontal" role="form"  method="post" enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input type="text" class="col-xs-10 col-sm-5" id="form-input-readonly" value="用户名不可更改" disabled="disabled">
							...
							<div class="col-xs-12">
								...
								<form class="form-horizontal" role="form"  method="post" enctype="multipart/form-data">
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input type="text" name="birthday" id="add_username" placeholder="用户名 " class="col-xs-10 col-sm-5">
										</div>
									</div>
									...
									<div class="form-group has-info">
										<label class="control-label col-xs-12 col-sm-3 no-padding-right" for="password">Password:</label>
											...
											<div class="col-xs-12 col-sm-9">
											<div class="clearfix">
											<input type="password" name="password" id="add_password" class="col-xs-12 col-sm-4 valid" placeholder="请输入新密码">
											</div>
											</div>
											</div>
									...
									<div class="form-group has-error">
													...
													<div class="col-xs-12 col-sm-9">
												<div class="clearfix">
												<input type="password" name="password2" id="add_password2" class="col-xs-12 col-sm-4" placeholder="请重复密码">
											</div><div for="password2" class="help-block" style="display:none;">Please enter at least 5 characters.</div>
											</div>
										</div>
									...
									<div class="form-group">
																	<label class="control-label col-xs-12 col-sm-3 no-padding-right" for="platform">管理员类型</label>
									...
									<div class="form-group">
																	<label class="control-label col-xs-12 col-sm-3 no-padding-right" for="comment">tips(备注)</label>
...
<![endif]-->
		...
		function transform(a){
			...
			ThinkAjax.send('<?php echo U("Admin/Account/alterAccountView");?>','ajax=1&id='+a,complete,'result');
		}
		...
		function complete(data,status){
			...
			$('form-input-readonly').value = data[0].admin_name;
			
			$('list').innerHTML = '<span>修改用户 <B><i> '+data[0].admin_name+' </i></B>的密码</span>';
			}else{
				alert("操作错误 ");
			}
		}
...
<script language="JavaScript">
		<!--
		function updatePro(){
			ThinkAjax.send('<?php echo U("Admin/Account/alterAccount");?>','password='+$('password').value+'&password2='+$('password2').value+'&id='+$('updateId').value,complete2,'result2');
		}
...
<script language="JavaScript">
		...
		function deleteFame(a){
			ThinkAjax.send('<?php echo U("Admin/Account/deleteUser");?>','id='+a,complete3,'result3');
		}
...
<script type="text/javascript">
function addUser(){
	ThinkAjax.send('<?php echo U("Admin/Account/addUser");?>','add_username='+$('add_username').value+'&add_password='+$('add_password').value+'&add_password2='+$('add_password2').value+'&comment='+$('comment').value,complete4,'result4');
	
}

function complete4(data,status){
	if (status==1){
		alert("新增用户成功");
	}else if(status==2){
		alert("用户名已经存在！请更换用户名重试");
	}else if(status == 3){
		alert("输入不能为空");
	}else{
		alert("密码不一致");
